...
async function loginUserAnonymouslyHandler(data: {
  language: string;
  username: string;
...
async function getUserInfo(data: { language: string }, context: any) {
  let t;
  try {
    t = getTranslation(data.language);
...
async function getUserInfoById(userId: string, language: string): Promise<any> {
  let t;
  try {
 ...
 */
async function checkEmailExistsHandler(data: {
  email: string;
 ...
 */
async function startFreeTrialHandler(
  data: any,
