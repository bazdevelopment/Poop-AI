 ...
 */
async function createActivityLogHandler(
  ...
  try {
    const { newStreak } = await db.runTransaction(async (transaction) => {
      ...
      const finalLog: ActivityLogDocument = {
        ...
        type: data.type,
        overcome: data.type !== 'excuse_logged_daily_checkin',
        status,
 ...
 */
async function getCalendarActivityLogHandler(
  data: RequestData,
  context: functions.https.CallableContext,
 ...
 */
async function updateActivityLogHandler(
