...
async function sendUserPushNotification(data: INotificationPayload, context: any) {
  let t;
  try {
...
async function handleSendGlobalPushNotifications(data: INotificationPayload, context: any) {
  let t;
  try {
    t = getTranslation(data.language);
...
async function handleGetUserNotification(data: {
  userId: string;
  language: string;
  lastDocId: string;
  limit?: number;
...
async function handleMarkNotificationAsRead(data: { notificationId: string }, context: any) {
  try {
