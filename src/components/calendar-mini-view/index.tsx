...
import { PoopIcon } from '../ui/assets/icons/poop';
...
function StreakDisplayHorizontal({ currentStreak }: { currentStreak: number }) {
  return (
    <View className="items-center justify-center self-stretch rounded-2xl border border-1 border-charcoal-600 p-3">
...
function StreakDisplayVertical({ currentStreak }: { currentStreak: number }) {
  return (
    <View className="mb-6 flex-row items-center gap-1 rounded-2xl bg-[#191A21] px-4 py-3">
      <View className="items-center justify-center rounded-xl bg-linear-to-br from-orange-400 to-orange-600 p-3">
        <StreakIcon width={40} height={40} />
      </View>
      <View>
        ...
        <Text className="font-medium-poppins text-sm text-gray-400">
          streak
        </Text>
      </View>
    </View>
        ...
        {status === 'inactive' ? (
          ...
          <View className="relative">
            ...
            {(status === 'completed' || status === 'attended') && (
              <View className="absolute -top-1 -right-1 size-4 items-center justify-center rounded-full bg-orange-600">
                <Ionicons name="checkmark-sharp" size={8} color="white" />
              </View>
            )}
            {(status === 'special' || status === 'streak_reset') && (
              <View className="absolute -top-1 -right-1 size-4 items-center justify-center rounded-full bg-blue-600">
...
function CalendarMiniView({
  ...
  if (layout === 'horizontal') {
    return (
      <View
        className={`flex-row items-center rounded-2xl ${containerClassName}`}
      >
