 ...
 */
export async function requestInAppRating(): Promise<RatingResult> {
  try {
 ...
 * Call this once after user completes onboarding or any positive action
 */
export async function requestInAppRatingAndStore(): Promise<RatingResult> {
  try {
    // Check if we should request rating
    const { canRequest, reason } = shouldRequestInAppRating();
 ...
 */
export function shouldRequestInAppRating(): {
  canRequest: boolean;
  reason?: string;
} {
 ...
 */
export function markUserHasRated(): void {
 ...
 */
export function requestAppRatingWithDelay(delayMs: number = 2000): void {
  setTimeout(async () => {
 ...
 * Request rating with a delay (good UX after completing onboarding)
 */
export function requestAppRatingWithDelayStorage(delayMs: number = 2000): void {
  setTimeout(async () => {
