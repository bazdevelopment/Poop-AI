...
type ISendOtpCodeVariables = {
  email: string;
  language: string;
};

type IValidateAuthCode = {
  authenticationCode: string;
  email: string;
  language: string;
};

export function useCreateAnonymousAccount(
...
export function useLoginWithEmail(variables: { email: string }) {
...
export function useUser(language: string) {
  return createQuery<Response, any, AxiosError>({
    queryKey: ['user-info'], // Include variables in the queryKey
    fetcher: () => getUserInfo({ language }), // Pass variables to the fetcher function
  })();
}

export function useSendVerificationCode({ email }: { email: string }) {
...
export function useValidateAuthCode() {
  const { logEvent, recordError } = useCrashlytics();
...
export function useDecrementScans() {
  const { logEvent, recordError } = useCrashlytics();
