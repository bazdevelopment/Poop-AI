...
export function useAllUserConversations(limit: number = 10) {
  return createQuery({
...
export function useGetAllUserConversations({
  userId,
  limit,
}: {
  userId: string;
...
type StreamingMessageParams = {
  onStream?: (chunk: string) => void;
  onComplete?: (fullResponse: string) => void;
  onError?: (error: Error) => void;
} & SendMessageParams;

export function useSendStreamingMessage({ onComplete, onError }) {
