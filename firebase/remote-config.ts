...
export async function checkForAppUpdate(minVersionAllowed: string) {
  try {
    // Compare versions to determine if an update is required
    const isUpdateRequired = compareVersions(
      minVersionAllowed,
...
function compareVersions(minVersion: string, currentVersion: string): boolean {
